...
step library pstate = trace ((show $ sum $ S.map shift out) ++ " " ++ (show (S.size out)) ++ " " ++ (show (S.findMin out)) ++ " " ++ (show (sum out))) out where
	shift x = x - S.findMin out
	out = S.fromList $ filter potWillGrow $ range
...
doTheThing (pstate, library) = S.findMin ( appEndo (fold (replicate 400 (Endo $ step library))) pstate) where
	-- length x = S.findMax x - S.findMin x
