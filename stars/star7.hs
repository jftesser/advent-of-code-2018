...
mostSleepyGuardSleepsMostAt :: GuardSleep -> Maybe (Guard, Int)
mostSleepyGuardSleepsMostAt x = getMostCommonMin <$> M.foldlWithKey f Nothing x where
	f (Just (currGuard, currSleepmins)) guard sleepmins = pure $ if MS.size currSleepmins > MS.size sleepmins then (currGuard, currSleepmins) else (guard, sleepmins)
	f Nothing guard sleepmins = pure (guard, sleepmins)
	getMostCommonMin :: (Guard, MS.MultiSet Int) -> (Guard, Int)
	getMostCommonMin (guard, sleepset) = (guard, fst $ MS.foldOccur f (0,0) sleepset) where
		f minute occ (oldmin, oldocc) = if occ > oldocc then (minute, occ) else (oldmin, oldocc)

sumFile :: IO ()
sumFile = (show <$> ( ((mostSleepyGuardSleepsMostAt . mapSleepToGuards . convertToShifts . sortList) <$>) <$> parseList <$> readListFile)) >>= putStrLn 
